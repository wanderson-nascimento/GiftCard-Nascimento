...
const metricsMiddleware = ({ metrics, serverTiming, name }) => {
    const serverTimingStart = process.hrtime();
    const serverTimingLabel = (0, utils_1.shrinkTimings)((0, utils_1.formatTimingName)({
        hopNumber: 0,
    ...
    return async (ctx, next) => {
        ...
        try {
            ...
            if (ctx.config.metric && ctx.response && ctx.response.status) {
                status = (0, status_1.statusLabel)(ctx.response.status);
            }
        }
        catch (err) {
            ...
            if (ctx.config.metric) {
                ...
                if (err.code === 'ECONNABORTED') {
                    ...
                    status = (0, status_1.statusLabel)(err.response.status);
                }
                else if (isCancelled) {
                    status = 'cancelled';
                }
            ...
            if (ctx.config.metric && metrics) {
                ...
                if (ctx.config.verbose) {
                    console.log(`VERBOSE: ${name}.${ctx.config.label}`, {
                        ...
                        millis: end
                            ? (0, utils_1.hrToMillis)(end)
                            : extensions.revalidated || extensions.router || status !== 'success'
                                ? (0, utils_1.hrToMillis)(process.hrtime(start))
                                : '(from cache)',
            ...
            if (serverTiming) {
                ...
                const dur = (0, utils_1.hrToMillis)(process.hrtime(serverTimingStart));
