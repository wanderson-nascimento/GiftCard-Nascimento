...
exports.MultilayeredCache = void 0;
...
class MultilayeredCache {
    constructor(caches) {
        ...
        this.get = async (key, fetcher) => {
            ...
            const failedCaches = (0, ramda_1.slice)(0, successIndex, this.caches);
            const [firstPromise] = (0, ramda_1.map)(cache => cache.set(key, value, maxAge), failedCaches);
            await firstPromise;
            return value;
        };
        this.set = async (key, value, maxAge) => {
            const isSet = await Promise.all((0, ramda_1.map)(cache => cache.set(key, value, maxAge), this.caches));
            return (0, ramda_1.any)(item => item, isSet);
        };
        this.has = async (key) => {
            const hasList = await Promise.all((0, ramda_1.map)(cache => cache.has(key), this.caches));
            return (0, ramda_1.any)(item => item, hasList);
        };
