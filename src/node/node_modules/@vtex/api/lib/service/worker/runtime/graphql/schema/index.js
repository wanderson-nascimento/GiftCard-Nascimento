...
const mergeTypeDefs = (appTypeDefs, schemaMetaData) => [appTypeDefs, (0, typeDefs_1.nativeTypeDefs)(schemaMetaData), schemaDirectives_1.nativeSchemaDirectivesTypeDefs].join('\n\n');
const hasScalar = (typeDefs) => (scalar) => new RegExp(`scalar(\\s)+${scalar}(\\s\\n)+`).test(typeDefs);
const extractSchemaMetaData = (typeDefs) => {
    const scalars = (0, ramda_1.keys)(typeDefs_1.nativeResolvers);
    const scalarsPresentInSchema = (0, ramda_1.map)(hasScalar(typeDefs), scalars);
    return (0, ramda_1.zipObj)(scalars, scalarsPresentInSchema);
};
const makeSchema = (options) => {
    const { resolvers: appResolvers, schema: appSchema, schemaDirectives: appDirectives } = options;
    const appTypeDefs = appSchema || (0, fs_extra_1.readFileSync)('./service/schema.graphql', 'utf8');
    const schemaMetaData = extractSchemaMetaData(appTypeDefs);
    const executableSchema = (0, graphql_tools_1.makeExecutableSchema)({
        resolvers: {
            ...appResolvers,
