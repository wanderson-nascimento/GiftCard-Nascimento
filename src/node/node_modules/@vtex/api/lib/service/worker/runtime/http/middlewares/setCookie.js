...
exports.removeSetCookie = void 0;
...
Cookies dropped:
  ...
  ${keys.join('\n\t')}
`;
const findStr = (target, set) => (0, ramda_1.find)((a) => a.toLocaleLowerCase() === target, set);
...
const cookieKey = (cookie) => (0, ramda_1.compose)(ramda_1.head, (0, ramda_1.split)('='))(cookie);
const indexCookieByKeys = (setCookie) => (0, ramda_1.map)((cookie) => [cookieKey(cookie), cookie], setCookie);
async function removeSetCookie(ctx, next) {
    const { vtex: { logger } } = ctx;
    await next();
    const setCookie = ctx.response.headers['set-cookie'];
    if (!setCookie || (0, ramda_1.isEmpty)(setCookie)) {
        return;
    }
    ...
    if (scope === 'public') {
        ...
        const cookies = (0, ramda_1.reduce)((acc, [key, payload]) => {
